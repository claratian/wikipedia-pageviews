class ZeroOrNotLoadedDataException(Exception):
	...
	def __str__(self):
		if isinstance(self.date, list):
			return "0 pageviews or data not available for date range {dates}".format(dates=str(self.date))
		else:
			return "0 pageviews or data not available for date {date}".format(date=str(self.date))
