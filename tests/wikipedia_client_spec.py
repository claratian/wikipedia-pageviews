...
class WikipediaClientTest(unittest.TestCase):
	...
	def test_views_per_article_by_day(self, mock_requests):
		...
		calls = [call(api_url.format(article=article, start=date, end=date), headers=HEADERS) for date in dates]
		mock_requests.assert_has_calls(calls, any_order=True)
	...
	def test_views_per_article(self, mock_requests):
		...
		mock_requests.assert_called_with(api_url.format(article=article, start=start_date, end=end_date), headers=HEADERS)
	...
	def test_zero_data_exception(self):
		...
		with self.assertRaises(Exception) as context:
			wikipedia_client.views_per_article_by_day("foo", invalid_per_article_dates, "daily")	
		self.assertEqual(len(eval(str(context.exception))), 2)

		self.assertRaises(ZeroOrNotLoadedDataException, lambda: wikipedia_client.views_per_article("foo", "20030701", "20030731", "monthly"))
	...
	def test_throttling_exception(self, mock_requests):
		...
		self.assertRaises(ThrottlingException, lambda: wikipedia_client.top_articles(["2023/07/01"]))
		self.assertRaises(ThrottlingException, lambda: wikipedia_client.views_per_article_by_day("foo", ["20230707"], "daily"))
		self.assertRaises(ThrottlingException, lambda: wikipedia_client.views_per_article("foo", "20230701", "20230731", "monthly"))
