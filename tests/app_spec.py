...
class WeeklyViewsPerArticleTest(unittest.TestCase):
	...
	def test_get_views_per_article(self, mock_views_per_article, mock_response):
		mock_data = {"items": [{"article": "foo", "views": 11}, {"article": "foo", "views": 12}]}
		mock_views_per_article.return_value = mock_data
		response_start_date = date(2023, 7, 1)
		response_end_date = date(2023, 7, 7)
		mock_response.return_value = response.ArticleViewsResponse(mock_data, "foo", response_start_date, response_end_date)
	    
		result = app.weekly_views_per_article("foo", '2023-07-01')

		mock_views_per_article.assert_called_with("foo", '20230701', '20230707', 'daily')
		mock_response.assert_called_with(mock_data, "foo", response_start_date, response_end_date)
...
class MonthlyViewsPerArticleTest(unittest.TestCase):
	...
	@patch("wikipedia_client.views_per_article")
	def test_get_views_per_article(self, mock_views_per_article, mock_response):
		mock_data = {"items": [{"article": "foo", "views": 10}, {"article": "foo", "views": 12}]}
		mock_views_per_article.return_value = mock_data
		response_start_date = date(2023, 7, 1)
		...
		mock_views_per_article.assert_called_with("foo", '20230701', '20230731', 'monthly')
		mock_response.assert_called_with(mock_data, "foo", response_start_date, response_end_date)
		self.assertEqual(result, mock_response.return_value)
...
class TopWeeklyViewsTest(unittest.TestCase):
	...
	def test_get_top_articles(self, mock_top_articles, mock_response):
		mock_data = [{'items': [{'articles': [{'article': "foo", "views": 4}, {'article': "bar", 'views': 2}]}]}]
		mock_top_articles.return_value = mock_data
		response_start_date = datetime(2023, 7, 1)
		response_end_date = datetime(2023, 7, 7)
		mock_response.return_value = response.TopViewsResponse(mock_data, response_start_date, response_end_date)
		...
		mock_response.assert_called_with(mock_data, response_start_date, response_end_date)
		self.assertEqual(result, mock_response.return_value)
...
class DayOfMonthWithMostViewsTest(unittest.TestCase):
	...
	def test_get_views_per_article_by_day(self, mock_views_by_day, mock_response):
		mock_data = [{'items': [{'timestamp': '2023070500', 'views': 6}]}, {'items': [{'timestamp': '2023070200', 'views': 4}]}]
		mock_views_by_day.return_value = mock_data
		mock_response.return_value = response.DateWithMostViewsResponse(mock_data, "foo")

		result = app.day_of_month_with_most_views("foo", "July")

		dates = ["20230701", "20230702", "20230703", "20230704", "20230705", "20230706", "20230707", "20230708", "20230709", "20230710", "20230711", "20230712", "20230713", "20230714", "20230715", "20230716", "20230717", "20230718", "20230719", "20230720", "20230721", "20230722", "20230723", "20230724", "20230725", "20230726", "20230727", "20230728", "20230729", "20230730", "20230731"]
